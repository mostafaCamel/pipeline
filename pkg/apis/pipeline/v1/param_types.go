...
type Param struct {
	...
	// The value will be obtained from the source during the TaskRun or PipelineRun initialization (instead of being fetched during the actual execution)
	// +optional
...
func (paramValues *ParamValue) UnmarshalJSON(value []byte) error {
	...
	if string(value[:]) == string(marshalledNil[:]) {
