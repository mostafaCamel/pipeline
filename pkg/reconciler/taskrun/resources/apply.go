...
func getParamValue(valInConfigMap string) (*v1.ParamValue, error) {
	...
	if err := json.Unmarshal([]byte(stringToUnmarshal), &newParamValue); err != nil {
		return nil, fmt.Errorf("encountered an error while converting the value '%s' obtained from the value source to a ParamValue: %v", valInConfigMap, err)
	}
...
func getConfigMap(ctx context.Context, k8s kubernetes.Interface, namespace string, configMapName string) (*corev1.ConfigMap, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("encountered an error while trying to get the configmap '%s' in the namespace '%s': %v", configMapName, namespace, err)
	}
